...
**Update (2026-05-29) — debt closed:** PRD-04 / issue #29 landed the owed coverage. The director's pure decisions were lifted into `scripts/SpawnPlan.gd` (weighted pick, concurrency cap, score-driven interval ramp, aimed-gap, one-shot boss trigger, tier-based aimed classifier) and the intro player's scheduling into `scripts/IntroSchedule.gd` (which events are due at elapsed `t`, finish-time), each with GUT tests in `test/unit/` (`test_spawn_plan.gd`, `test_intro_schedule.gd`, `test_spawn_director_classifier.gd`). The filename-based classifier is gone — the director now reads each enemy's declared `primary_threat_tier`. The specific SpawnDirector/StageIntroPlayer test debt this ADR named is **paid**.
