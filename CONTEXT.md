...
**Enemy archetype**:
A behaviour-first enemy role (movement + threat tier), reskinned with biome-appropriate art and drawn from the **procedural body**'s spawn table. The current set: **Strafer** (air, straight — formation fodder), **Diver** (air, aimed — dives at the player), **Gunship** (air, **pattern** — spread/burst flyer; purple, so boss-/elite-tier and not seen before City/L3), **Emplacement** (static ground, aimed — tracks & fires while scrolling past), **Tank** (mobile ground, aimed — turret tracks, fires shells), **Convoy** (ground, little/no threat — coin reward), **Warship** (naval, straight+aimed — high HP, multi-turret), **Elite** (any domain — see below).

_Realization note (the names above are the behavioural vocabulary; the shipped scenes keep asset-names, each tagged with a `primary_threat_tier`):_ Strafer ≈ `Fighter`, Diver ≈ (not yet realized as its own scene), Gunship ≈ `Bomber` / `AceFighter` / `Interceptor`, Emplacement ≈ `RocketLauncher` / static `Tank`, Tank ≈ `Tank`, Convoy ≈ `Truck` / `Train`, Warship ≈ `Ship`, Elite ≈ `EliteEscort`. Scenes are *not* renamed to the canonical names (kept clean per the N=1 anti-abstraction rule); this mapping is the bridge. Two known tag defects to fix in their own level PRDs: `EliteEscort` is tagged AIMED but an Elite is **pattern/purple** by definition, and no `Diver` scene exists yet.
